...
>A minus sign was added to the code to exclude the first and third values in the vector.
...
>The first line is specifying to print only those values that have been assigned the logical value of TRUE, while those labeled FALSE are omitted. The next line of code is to print only the values that are greater than 7. 
...
The above code assigned letters to the first four values in the vector, and the subsequent line pulled the value that was assigned "d."
...
>The first code generated all rows contained within two of the fourteen columns in the dataset, while the subsequent code produced just the first three rows. 
...
```
```{r}
statepopulations[67,2:28]
```

>I wanted to see the history of population growth for my home state, so I chose to subset a single row. 
...
>The operator specifies city names as the variable we're pulling, and it's only be pulled from a limited number of rows (100-110).
...
```{r}
p <- c("Kennedy", "Biden", "Reagan", "Carter", "Nixon", "Roosevelt")
if (p[1] > 5) print(TRUE) else print(FALSE)
```
...
>I first determined what code would be needed to determine the most recent date in the dataset. I then plugged that code into an if-statement to determine if that date was before or after 1990. 
...
>This specifies that the action is to be applied to each entry in the vector.
...
```{r}
courses <- c("Studio I", "Studio II", "Digital Methods for History")

for (c in 1:length(courses)) {
  print(paste("I'm enrolled in", courses[c]))
}
```
...
>The code specifies to skip any values less than 3 and to exit the loop before reaching any values equal to, or greater than, 5. 
...
```{r}
test<-statepopulations[!is.na(statepopulations$X1800),]

for (i in 1:length(test$X1800)) {
  if (test$X1800[i] > 200000 & test$X1800[i] < 250000) 
...
```{r}
historyStudents("David Williams", "Digital Methods for History")
```
...
>I consulted our friends at Stack Overflow for some guidance after my first few attempts failed. I was simply placing "nchar" in the wrong location of my code--I had it confused with my input function--which is why I wasn't getting any results. I assigned a value to "w" (i.e., my string) and wrote the code to count characters ("nchar") and paste the result within the statement specified in the instructions.
...
>The grade ("C") has been assigned to George Washington as a value. It is not, in this particular case, a return value. 
...
```{r}
title_finder <- function (city, year) {
  data(gayguides)
...
```
```{r}
title_finder("Seattle", 1981)
```
>I'm receiving an error message when I try to run the function: "unexpected string constant." I've tried to resolve the issue but am unable to idenfity the flaw in my coding. I apologize for this. 
...
```{r}
voter_records <- function(x) {
  data("BostonWomenVoters")
  
for (i in 1:length(BostonWomenVoters$Occupation)) {
  if (BostonWomenVoters$Occupation[i] == x) {
...
voter_records("Box maker")
```
>Regrettably, I was unable to arrive at a code that would produce an easily readable output. The above code is a start, but it needs to be refined somewhere. That's where I'm stumped!  
