...
```{r}
"I am excited to be coding for the first time"
```
...
```{r}
paste("My name is David", "and I love movies")
```

(@) Can you explain what happened in 2-3 sentences? 

> I used the paste function to tie together two strings and form a complete sentence. I used a comma to separate those strings.  
...
>The code on line 98 suggests that 5 and 10 are equal numbers, the code on 99 states that 4 is a greater value than 3, the code on 100 states that 5 and 10 are not equal numbers, the code on 101 states that 5 and 5 or not equal, the code on 102 states that 3 is equal to either 4 or 3, and the code on 103 states that 3 is not equal to 4 and that 3 is not equal to 5.
...
```
```{r}
d <- "I might just get the hang of it"
```
paste(c, d)
```{r}
paste(c, d)
...
>The code took each number in the vector list and muliplied it by a factor of 2. This resulted in four new figures being generated.
...
>The third number in the vector list was multiplied by a factor of 2.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
RCID_coders <- c("Lane", "David", "Justin", "Jared")
```
...
>This function takes a single numeric argument x and returns a numeric vector containing the largest integars not greater than the corresponding elements of x. 
...
>Column Z multiplies each number in the X column by a factor of 2 and then adds it to the Y value on that same line.
...
>There are 14 variables and 60698 observations. The description states that the dataset is based on information dating from 1965 to 1985. 
...
>I consulted Google to see how the code should be modified. 
...
>The vector "gayguides" identifies the dataset we are trying to obtain information about, the operator "$" allows us to specify one variable within said dataset, "title" is the variable seelected, and "500" refers to the 500th item.
...
>This dataset contains information about newly registered women voters after the passage of the 19th Amendment. It remains in progress as more registers are being transcribed. The dataset contains 10702 observations of 43 variables and spans from October to November 1920.
