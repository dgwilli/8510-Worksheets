...
>I've added annotations throughout to explain my process and decisions. I know the worksheet question does not explicity ask us to do this, so I hope it's OK!

(@) What happens if you create a custom stopword list? How does this change the model?
```{r}
file_paths <- list.files("txt/") 
MindAndBody <- readtext(paste("txt/", file_paths, sep=""))
MindAndBody$doc_id <- gsub("MB_", "", MindAndBody$doc_id)
MaB <- full_join(metadata, MindAndBody, by = c("Filename" = "doc_id"))
MaB_whole <- MaB %>%
  unnest_tokens(word, text) %>%
  filter(str_detect(word, "[a-z']$")) %>% 
  anti_join(stop_words)
...
```{r}
MaB.tf.idf <- MaB_whole %>%
  count(Filename, word, sort = TRUE)  %>%  
  bind_tf_idf(word, Filename, n) %>%
  arrange(tf_idf) #I want to measure the frequency of words across the entire corpus of texts to see which ones I want to add to stop_words. 
```
```{r}
MaB_terms <- MaB.tf.idf %>% filter(tf_idf == 0) #This line of code is intended to help me identify which words appear most often throughout the entire corprus of texts. I want to see if this will yield more nuanced results by weeding out more general terms that were likely to appear in all issues of Mind and Body. 
```

```{r}
Extra_stop_words <- c(MaB_terms$word) #I now take the terms identified by the previous line of code and turn them into a character vector and assign it a value.  
```

```{r}
Custom_stop_words <- stop_words %>% add_row(word=c(Extra_stop_words), lexicon="NA") #Now, I'm adding that vector to the default list of stop words, increasing its total from around 1,100 to over 8,000. This is my custom list. 
```
...
```{r}
MaB_words <- MaB_whole %>% count(Filename, word)
```
...
```{r}
MaB.lda <- LDA(MaB.dtm, k = 25, control = list(seed = 12345))
MaB.lda
```

```{r}
MaB.topics <- tidy(MaB.lda, matrix = "beta")
MaB.topics
```
```{r}
MaB.top.terms <- MaB.topics %>%
  group_by(topic) %>%
  top_n(3, beta)
```
...
```{r}
MaB_docs <- tidy(MaB.lda, matrix = "gamma")
MaB_docs
```

```{r}
MaB_docs %>%  filter(document == "1917_Sept.txt") %>% arrange(desc(gamma))
```

```{r}
Mab_wordcloud <- MaB.topics %>% filter(topic == 2)
wordcloud(topic1$term, topic1$beta, max.words = 40, random.order = FALSE,
    rot.per = 0.3, colors = brewer.pal(4, "Dark2"), scale = c(2, .5)) #I thought it might be beneficial to create a wordcloud for a single topic to see what observation(s) I might come up with. What I find interesting is the prominence of the term "playground," which might warrant a closer examination as to how often, and in what context, this publication discussed schools. 
```


>There appears to be more diversity and precision among the listed terms. There is very little repetition. "Theory," "activities," "lungs," "dancing," and "habits" seem especially noteworthy, as they offer more specificity than "education" and "physical," which rank near the top of the most frequently appearing terms (before they're added to stop_words). Customzing the stop_words simply allows me to widdle down the results that much more.  
...
```
```{r}
MaB_20s_words <- MaB_1920s %>% count(Filename, word)

MaB1920s.dtm <- MaB_20s_words %>% 
  count(Filename, word) %>% 
  cast_dtm(Filename, word, n)

MaB1920s.lda <- LDA(MaB1920s.dtm, k = 35, control = list(seed = 12345)) #I wanted to see if increasing the number of topics would yield even more specific terms that I could work with. 
MaB1920s.lda

MaB20_topics <- tidy(MaB1920s.lda, matrix = "beta")
MaB20_topics
```
```{r}
MaB1920s_top_terms <- MaB20_topics %>%
  group_by(topic) %>%
  top_n(5, beta)
...
```{r}
MaB1920s_docs <- tidy(MaB1920s.lda, matrix = "gamma")
MaB1920s_docs
```
>Narrowing my topic model down to a single decade gives me a clearer picture of which topics were most in vogue during that time. Specifically, two terms that pop up during this topic model that didn't appear in the previous one were "anthropometrics" (the study of variations in the sizes of human bodies) and "heredity." Given how prominent the controversial field of eugenics was in society and medicine at the time--to my understanding, the height of eugenics was from the late nineteenth century to about the 1950s--it would be worthwhile to look and see if any issues of Mind and Body tap into this conversation. 
